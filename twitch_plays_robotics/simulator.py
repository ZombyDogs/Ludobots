...
class SIMULATOR:
	...
	def __init__(self,playBlind=False,playPaused=False,evaluationTime=100):
		...
		self.simulator = Popen(commandsToSend, stdout=PIPE, stdin=PIPE, stderr=PIPE)
                ...
                self.Send('EvaluationTime '+str(self.evaluationTime)+'\n')

        def Get_Sensor_Data(self,ID,s):

                return self.dataFromPython[ID,s,:]
